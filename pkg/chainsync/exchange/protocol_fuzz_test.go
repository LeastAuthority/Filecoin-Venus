...
func TestFuzzBSTipset_raw(t *testing.T) {
	_, panics, _ := fleece.
		MustNewCrasherIterator(env, FuzzBSTipset_raw, filters...).
		TestFailingLimit(t, crashLimit)

	require.Zero(t, panics)
}
func TestFuzzRequest_raw(t *testing.T) {
	_, panics, _ := fleece.
		MustNewCrasherIterator(env, FuzzRequest_raw, filters...).
		TestFailingLimit(t, crashLimit)

	require.Zero(t, panics)
}

func TestFuzzRequest_structured(t *testing.T) {
	_, panics, _ := fleece.
		MustNewCrasherIterator(env, FuzzRequest_structured, filters...).
		TestFailingLimit(t, crashLimit)
